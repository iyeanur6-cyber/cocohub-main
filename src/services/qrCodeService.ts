...
export const sharePetQRCode = async (
  ...
  await Share.share({
    title: `${petData.name}'s Cocohub QR`,
    message: `Cocohub QR for ${petData.name}:\n${imageUrl}`,
    url: imageUrl,
  });
  return payload;
};
...
export const printPetQRCode = async (
  ...
  const imageUrl = getQRImageUrl(payload);
  await Share.share({
    title: `Print ${petData.name}'s Cocohub QR`,
    message: `Print or save this Cocohub QR for ${petData.name}:\n${imageUrl}`,
    url: imageUrl,
  });
  return payload;
};
 ...
 *  - valid: whether the QR is a genuine Cocohub code
 *  - petId: the pet's ID
