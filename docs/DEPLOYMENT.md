# Cocohub Mobile — Deployment Guide

This document covers the complete CI/CD lifecycle for Cocohub Mobile: how builds are triggered,
...
Cocohub uses **Expo Application Services (EAS)** to build and sign iOS and Android binaries in
...
| Secret | Description |
|---|---|
| `APPLE_ID` | Your Apple ID email (e.g. `dev@cocohub.app`) |
...
(`https://staging.cocohub.app/api`).

**App identifiers for staging:**

| Platform | Identifier |
|---|---|
| iOS | `app.cocohub.mobile.staging` |
| Android | `app.cocohub.mobile.staging` |
...
```bash
APP_ENV=staging
STAGING_API_URL=https://staging.cocohub.app/api
...
2. Enable **Incoming Webhooks** and add a webhook for your `#cocohub-ci` channel.
